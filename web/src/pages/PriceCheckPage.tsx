...
export function PriceCheckPage() {
  ...
  const [slug, setSlug] = useState(matchSlug(league.matches[0]));
  const match =
    league.matches.find((m) => matchSlug(m) === slug) ?? league.matches[0];
  const name = (n: string) => (lang === "zh" ? teamZh(n) : n);

  const selectClass =
    "min-h-11 w-full rounded-xl border border-line bg-raised px-3 text-sm text-text outline-none focus:border-lime";
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="block">
            ...
            <select
              ...
              onChange={(e) => {
                const next =
                  LEAGUES.find((l) => l.league === e.target.value) ?? LEAGUES[0];
                setLeagueSlug(next.league);
                setSlug(matchSlug(next.matches[0]));
              }}
